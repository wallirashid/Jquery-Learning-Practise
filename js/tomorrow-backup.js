jQuery(document).ready(function() {
    ...
    jQuery("#click-me").on("click", function() {
        jQuery("#first-ul").append(
            ...
            "</div>" +
            "<input type='text' placeholder='Enter your todo Task' class='full-width'><button class='btn-styling save-task' type='submit'>Add Task</button><span></span>" +
            "</li>"
        );
        totalListItems();
    });
    ...
    jQuery(document).on("click", "#first-ul input[type='checkbox']", function() {
        if (jQuery(this).is(":checked")) {
            ...
            jQuery("#destination").find(".tools i.fa.fa-edit").remove();
...
/*--------Save task when click on add button code -----------*/
...
/*jQuery(document).on("click",".fa.fa-edit",function(){
      ...
      if(jQuery(this).parents("li").has(".edit-text-field")){
        jQuery(this).parents("li").removeClass("edit-text-field");
      } else {
        var editLiText = jQuery(this).parents("li").append("<input type='text' placeholder='Edit Information' class='full-width'><button class='btn-styling edit-task' type='submit'>+Add Task</button><span></span>");
      }
      jQuery(this).parents("li").addClass("edit-text-field");
});*/

  jQuery(document).on("click",".fa.fa-edit", function() {
      var emptyString = "";
      var currentTxt = jQuery(this).parents("li").find(".text").text(emptyString);
      var editLiText = jQuery(this).parents("#first-ul li").append("<input type='text' placeholder='Edit Information' class='full-width'><button class='edit-task btn-styling' type='submit'>Add Task</button><span></span>");
  });